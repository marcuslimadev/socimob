...
import { Building2, ClipboardCheck, Download, FileText, Loader2, Pencil, Plus, Search, Trash2, Users, X } from 'lucide-react';
...
type Vistoria = {
  ...
  links_publicos?: { midias?: string | null; contestacao?: string | null; pdf?: string | null };
...
export default function Vistorias() {
  ...
  const downloadPdf = async (item: Vistoria) => {
    if (!item?.id) return;
    setDownloadingPdfId(item.id);
    try {
      const response = await api.get(`/vistorias/${item.id}/download-pdf`, { responseType: 'blob' });
      const blob = new Blob([response.data], { type: response.headers?.['content-type'] || 'application/pdf' });
      const url = window.URL.createObjectURL(blob);
      setPdfViewerTitle(item.codigo || `Vistoria #${item.id}`);
      ...
      console.error(error);
      toast.error(error?.response?.data?.message || 'Gere o laudo antes de visualizar o PDF.');
    } finally {
      setDownloadingPdfId(null);
    }
  };

  const generatePdf = async (item: Vistoria, shouldDownload = true) => {
    if (!item?.id) return;
    setGeneratingPdfId(item.id);
    try {
      const { data } = await api.post(`/vistorias/${item.id}/gerar-pdf`);
      const updated = data.vistoria as Vistoria;
      setVistorias((prev) => prev.map((row) => (row.id === item.id ? { ...row, ...updated } : row)));
      toast.success('Laudo PDF gerado.');
      if (shouldDownload) {
        await downloadPdf({ ...item, ...updated });
      }
    } catch (error: any) {
      console.error(error);
      toast.error(error?.response?.data?.message || 'Não foi possível gerar o laudo PDF.');
    } finally {
  ...
  return (
    ...
    <div className="page-header"><div><h1 className="page-title mb-2">Central de Vistorias</h1><p className="page-subtitle">Módulo próprio para operar visitas ao imóvel: use o cadastro quando existir ou descreva o local só aqui.</p>{isFieldUser ? <p className="mt-3 rounded-xl border border-cyan-500/20 bg-cyan-500/5 px-3 py-2 text-xs leading-relaxed text-muted-foreground sm:text-sm"><span className="font-medium text-foreground">Área operacional:</span> marque <strong className="text-foreground">Minhas visitas</strong> quando seu usuário estiver vinculado a uma pessoa, ou use <strong className="text-foreground">Filtrar por responsável</strong> nos filtros avançados.</p> : null}</div><div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row"><Link to="/vistorias/wizard" className="flex min-h-11 touch-manipulation items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-cyan-500 px-5 py-3 text-base font-semibold text-white sm:min-h-10 sm:text-sm"><Plus size={18} aria-hidden />Nova vistoria</Link><Link to="/vistorias/solicitacoes" className="flex min-h-11 touch-manipulation items-center justify-center rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-center text-base font-medium text-foreground sm:min-h-10 sm:text-sm">Solicitações</Link><Link to="/vistorias/contestacoes" className="flex min-h-11 touch-manipulation items-center justify-center rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-center text-base font-medium text-foreground sm:min-h-10 sm:text-sm">Contestações</Link></div></div>
    ...
    <div className="glass-panel rounded-2xl p-5">{loading ? <div className="flex items-center justify-center py-16 text-muted-foreground"><Loader2 className="mr-2 h-6 w-6 animate-spin" />Carregando vistorias...</div> : vistorias.length === 0 ? <div className="rounded-2xl border border-dashed border-white/10 px-6 py-14 text-center text-sm text-muted-foreground">Nenhuma vistoria encontrada.</div> : <div className="grid gap-4 xl:grid-cols-2">{vistorias.map((item) => <div key={item.id} className="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-5"><div className="mb-4 flex flex-col gap-4"><div className="min-w-0"><div className="mb-2 flex flex-wrap items-center gap-2"><span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${badgeClass(item.status)}`}>{statusOptions.find(([value]) => value === item.status)?.[1] || item.status}</span><span className="rounded-full border border-white/10 px-2.5 py-1 text-xs text-muted-foreground">{tipoOptions.find(([value]) => value === item.tipo)?.[1] || item.tipo || '-'}</span>{item.pdf_path ? <span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 text-xs text-emerald-200">PDF gerado</span> : null}</div><h3 className="break-words text-lg font-semibold leading-snug text-foreground">{item.codigo || `Vistoria #${item.id}`}</h3><p className="mt-1 text-sm text-muted-foreground">{item.cliente_nome || item.contrato?.locatario?.nome || 'Sem cliente vinculado'}</p></div><div className="grid w-full grid-cols-2 gap-2 sm:grid-cols-3 2xl:grid-cols-5"><button type="button" onClick={() => startFromList(item)} disabled={startingId === item.id || item.status === 'cancelada'} className={`inline-flex min-h-10 w-full min-w-0 touch-manipulation items-center justify-center rounded-xl border px-2.5 py-2 text-center text-xs font-medium disabled:opacity-50 ${startActionClass(item.status)}`}>{startingId === item.id ? 'Iniciando...' : startActionLabel(item.status)}</button><button type="button" onClick={() => generatePdf(item, true)} disabled={generatingPdfId === item.id} className="inline-flex min-h-10 w-full min-w-0 touch-manipulation items-center justify-center gap-1 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-2 text-xs font-medium text-emerald-200 disabled:opacity-60">{generatingPdfId === item.id ? <Loader2 size={14} className="animate-spin" aria-hidden /> : <FileText size={14} aria-hidden />}<span>{generatingPdfId === item.id ? 'Gerando...' : 'Gerar laudo'}</span></button><button type="button" onClick={() => downloadPdf(item)} disabled={!item.pdf_path || downloadingPdfId === item.id} className="inline-flex min-h-10 w-full min-w-0 touch-manipulation items-center justify-center gap-1 rounded-xl border border-white/10 bg-white/5 px-2.5 py-2 text-xs font-medium text-foreground disabled:cursor-not-allowed disabled:opacity-40">{downloadingPdfId === item.id ? <Loader2 size={14} className="animate-spin" aria-hidden /> : <Download size={14} aria-hidden />}<span>Ver PDF</span></button><button type="button" onClick={() => { openEdit(item); setShowForm(true); }} className="inline-flex min-h-10 w-full min-w-0 touch-manipulation items-center justify-center gap-1 rounded-xl border border-blue-500/30 bg-blue-500/10 px-2.5 py-2 text-xs font-medium text-blue-300"><Pencil size={14} aria-hidden /><span>Editar</span></button>{isAdminLike ? <button type="button" onClick={() => setDeleteId(item.id)} className="inline-flex min-h-10 w-full min-w-0 touch-manipulation items-center justify-center gap-1 rounded-xl border border-red-500/30 bg-red-500/10 px-2.5 py-2 text-xs font-medium text-red-300"><Trash2 size={14} aria-hidden /><span>Excluir</span></button> : null}</div></div><div className="grid gap-3 md:grid-cols-2"><div className="rounded-xl border border-white/10 bg-black/10 p-3"><p className="mb-1 text-xs uppercase tracking-[0.18em] text-muted-foreground">Contrato</p><p className="text-sm text-foreground">{contratoText(item.contrato)}</p></div><div className="rounded-xl border border-white/10 bg-black/10 p-3"><p className="mb-1 text-xs uppercase tracking-[0.18em] text-muted-foreground">Imóvel</p><p className="text-sm text-foreground">{imovelText(item.imovel || item.contrato?.imovel)}</p></div><div className="rounded-xl border border-white/10 bg-black/10 p-3"><p className="mb-1 text-xs uppercase tracking-[0.18em] text-muted-foreground">Responsável</p><p className="text-sm text-foreground">{personText(item.responsavel)}</p></div><div className="rounded-xl border border-white/10 bg-black/10 p-3"><p className="mb-1 text-xs uppercase tracking-[0.18em] text-muted-foreground">Execução</p><p className="text-sm text-foreground">{formatDate(item.data_vistoria)}</p><p className="mt-1 text-xs text-muted-foreground">{formatMetric(item.metragem)} • {item.mobiliado ? 'Mobiliado' : 'Sem mobília'}</p></div></div><div className="mt-4 rounded-xl border border-white/10 bg-black/10 p-3"><p className="mb-2 text-xs uppercase tracking-[0.18em] text-muted-foreground">Participantes</p><p className="text-sm text-foreground">{item.participantes_nomes?.length ? item.participantes_nomes.join(', ') : 'Sem participantes vinculados.'}</p></div><div className="mt-4 rounded-xl border border-cyan-500/20 bg-cyan-500/10 p-3"><p className="mb-1 text-xs uppercase tracking-[0.18em] text-cyan-100/80">Próxima ação recomendada</p><p className="text-sm text-cyan-100">{nextActionHint(item)}</p></div>{item.observacoes ? <p className="mt-4 text-sm leading-6 text-muted-foreground">{item.observacoes}</p> : null}</div>)}</div>}<div className="mt-6 flex items-center justify-center gap-3"><button type="button" onClick={() => setPage((prev) => Math.max(1, prev - 1))} disabled={page <= 1} className="min-h-10 touch-manipulation rounded-xl border border-white/10 px-4 py-2 text-sm text-foreground disabled:opacity-40">Anterior</button><span className="text-sm text-muted-foreground">Página {page} de {lastPage}</span><button type="button" onClick={() => setPage((prev) => Math.min(lastPage, prev + 1))} disabled={page >= lastPage} className="min-h-10 touch-manipulation rounded-xl border border-white/10 px-4 py-2 text-sm text-foreground disabled:opacity-40">Próxima</button></div></div>
        </motion.div>
      </div>
    </div>
  ...
  {pdfViewerUrl ? (
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-950/95">
      <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
        <div>
          <p className="text-sm font-semibold text-foreground">Laudo PDF</p>
          <p className="text-xs text-muted-foreground">{pdfViewerTitle}</p>
        </div>
        <button type="button" onClick={() => setPdfViewerUrl((previous) => { if (previous) window.URL.revokeObjectURL(previous); return null; })} className="rounded-xl border border-white/10 px-4 py-2 text-sm text-foreground">Fechar</button>
      </div>
      <iframe title="Laudo PDF" src={pdfViewerUrl} className="h-full w-full flex-1 bg-white" />
    </div>
  ) : null}
