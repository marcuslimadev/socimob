...
class VistoriaFotosApiController extends Controller
{
    ...
    public function store(Request $request, int $vistoriaId)
    {
        ...
        $foto = VistoriaFoto::create([
            ...
            'enviado_por_tipo' => $authUser?->role === 'admin' || $authUser?->role === 'super_admin' ? 'admin' : 'app',
        ]);
    ...
    public function destroy(Request $request, int $vistoriaId, int $fotoId)
    {
        $vistoria = $this->vistoriaForTenant($request, $vistoriaId);
        if (!$vistoria) {
            return response()->json(['success' => false, 'message' => 'Vistoria não encontrada'], 404);
