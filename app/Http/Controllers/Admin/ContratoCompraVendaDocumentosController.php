...
use App\Services\ContratoCompraVendaDocumentoService;
use App\Services\D4SignService;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class ContratoCompraVendaDocumentosController extends Controller
{
    public function __construct(
        private readonly ContratoCompraVendaDocumentoService $documentoService,
    ...
    {
        $contrato = ContratoCompraVenda::find($contratoId);
        if (!$contrato) {
            return response()->json(['success' => false, 'message' => 'Contrato não encontrado'], 404);
        }

        $items = ContratoDocumento::where('contrato_compra_venda_id', $contratoId)
            ->orderByDesc('versao')
    ...
    {
        $contrato = ContratoCompraVenda::with([
            'imovel',
            'vendedor',
            'comprador',
        ])->find($contratoId);
        ...
        $validator = Validator::make($request->all(), [
            'tipo' => 'required|string|in:compra_venda',
    ...
    {
        $documento = ContratoDocumento::where('contrato_compra_venda_id', $contratoId)->find($documentoId);
        if (!$documento) {
