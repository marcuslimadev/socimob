...
export default function Vistorias() {
  ...
  const downloadPdf = async (item: Vistoria) => {
    ...
    try {
      ...
      link.href = url;
      link.download = `${item.codigo || `vistoria-${item.id}`}.pdf`;
      ...
      console.error(error);
      toast.error(error?.response?.data?.message || 'Gere o laudo antes de baixar o PDF.');
    } finally {
